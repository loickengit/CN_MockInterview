<template>
  <div id="dashboard">
    ...
    <div class="row, text-center" style="font-family: 'Microsoft YaHei';font-size:20px;font-weight:bold;">
  ...
  export default {
    ...
    methods: {
      ...
      async selectInterview(){
        let uid = localStorage.getItem("USERID");
        this.$http.post('/api/user/selectInterview', {
          intervieweeId: uid
        },{}).then((response) => {
          for(let i=0;i<response.data.length;i++){
            ...
            if(subject!=""){
              this.tableData.push({"subject":subject,"name":name,"time":time,"state":state});
